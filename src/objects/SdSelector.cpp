...
void SdSelector::putToClipboard(const SdProject *project, double scale , SdWEditor::SdCopyFormat format)
  ...
  if( format == SdWEditor::SdCopyFormat::sdcfDefault ) {
    ...
    svgGenerator.setTitle( "SalixEDA SVG drawing" );
    svgGenerator.setDescription( "An SVG drawing created by the SalixEDA SVG Generator " );
    QPainter svgPainter;
    ...
    packedString.append( QStringLiteral("`SalixEDA compressed") );
...
SdProject *SdSelector::getFromClipboard()
  ...
  if( mime != nullptr && mime->hasText() ) {
    ...
    if( src.startsWith( QStringLiteral("SalixEDA compressed") )  ) {
      //This contens is SalixEDA compressed representation
