...
class SdModeCRoadEnter : public SdModeCommon
  {
    ...
    SdPvStratum          mStack;             //!< Available stratum stack at current point
  ...
  private:
    ...
    void            getNetOnPoint( SdPoint p, SdPvStratum s, QString &netName, SdPvStratum &destStratum );

    //If point is on middle of same road segment we split it on point
    void            splitRoadSegment( SdPoint p, SdPvStratum s, QString &netName, SdPvStratum &destStratum );
    ...
    void            changeToTraceLayer( SdPvStratum dest );

    //Add via to change trace layer
    void            addVia( SdPvStratum newStratum );

    //First point enter
    void            firstPointEnter( bool enter );

    //Find loop. If found it will be placed to mLoopPath
    void            findLoop( SdPoint src, SdPoint dst, SdPvStratum st );
