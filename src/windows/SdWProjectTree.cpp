...
bool SdWProjectTree::cmFileSaveAs()
  {
  QString extension;
  QString binaryFiles( tr("SalixEDA binary Files (*%1)").arg(SD_BINARY_EXTENSION) );
  QString textFiles( tr("SalixEDA text Files (*%1)").arg(SD_BASE_EXTENSION) );
  QString title = QFileDialog::getSaveFileName(this, tr("Save project file"), QString(), tr("%1;;%2").arg( binaryFiles, textFiles ), &extension );

  if( title.isEmpty() ) return false;

  mFileName = title;
  if( !mFileName.endsWith(SD_BASE_EXTENSION) && !mFileName.endsWith(SD_BINARY_EXTENSION) ) {
    if( extension == textFiles )
...
//    qDebug() << "activate project" << fileName();
  ...
  //MainMenu::cmObjectPaste->setEnabled( QApplication::clipboard()->mimeData()->hasFormat(SALIXEDA_CLIP_FORMAT_OBJECT) );
