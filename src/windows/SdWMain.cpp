...
void SdWMain::activateProjectName(SdProject *project)
  ...
  if( activeProject() && activeProject()->getProject() == project )
    setWindowTitle( QString( "SalixEDA v%1.%2 [%3%4]").arg(SD_VERSION_MAJOR).arg(SD_VERSION_MINOR).arg( project->isDirty() ? QChar('*') : QChar(' ') ).arg( activeProject()->fileName() ) );
  else
    setWindowTitle( QString( "SalixEDA v%1.%2").arg(SD_VERSION_MAJOR).arg(SD_VERSION_MINOR) );
...
void SdWMain::cmHelpAbout()
  ...
  QMessageBox::about( this, tr("About %1").arg(SD_NAME), tr("<html><body><p>Version %1.%2, </p>"
                                                            "<p>Copyright SalixEDA</p>"
                                                            "<p>Author: %3</p>"
                                                            "<p>Web:<br><a href=\"http://SalixEDA.org\">SalixEDA.org</a></p></body></html>").
...
void SdWMain::cmGuiderDialog()
  ...
  if( !scenaFile.isEmpty() ) {
    ...
    if( mGuiderCapture == nullptr ) {
      ...
      connect( mGuiderCapture, &SdGuiderCapture::stepIndexChanged, mGuiderDialog, &SdGuiderDialog::stepIndexChanged );
      connect( mGuiderCapture, &SdGuiderCapture::recordStatus, this, [this] ( bool isRecord ) {
        if( isRecord ) {
...
void SdWMain::cmGuiderSnapshotLoad(const QString &scriptPath, int snapshotIndex, const SdGuiderStepList &)
  {
  QString snapshotName("scena-%1.snapshot");
  QFile snapshotFile( scriptPath + snapshotName.arg(snapshotIndex) );
